...
void UIManager::setup() {
    ...
    builder.AddChar(0xf192); // circle (record)
    builder.AddChar(0xefba); // mouse (assist)
    builder.AddChar(0xf03d); // camera (render)
    builder.AddChar(0xf121); // scripting
    builder.AddChar(0xf013); // cog (settings)
    builder.AddChar(0xf078); // arrow down
    builder.AddChar(0xf044); // notebook and pencil (edit)
    builder.AddChar(0xf054); // arrow right
    builder.AddChar(0xf00c); // checkmark
    builder.AddChar(0xf51b); // gemini (prediction)
    builder.AddChar(0xf004); // heart
    ...
    tabby::window("Silicate", [this, bot, popupShaderFn]() {
        ...
        tabby::section(
            ...
            [&]() {
                ...
                if (tabby::button_selector(
                        "\uf121    Scripts##Tab",
                        m_state.m_currentTab == UIState::UITab::Scripts)
                        .pressed) {
                    m_state.m_currentTab = UIState::UITab::Scripts;
                }
            ...
            tabby::tab(m_state.m_currentTab, UIState::UITab::Scripts, [&]() {
                ...
                for (int i = 0; i < scripts.m_scripts.size(); i++) {
                    ...
                    tabby::checkbox("Enabled##Script" + std::to_string(i),
                                    script.m_enabled);
                    if (enabled != script.m_enabled) {
                        if (script.m_enabled) {
                            scripts.enableScript(script);
                        } else {
                            scripts.disableScript(script);
                        }
                    }

                    if (tabby::button("Unload##Script" + std::to_string(i)).pressed) {
                        scripts.unloadScript(i);
                    }
